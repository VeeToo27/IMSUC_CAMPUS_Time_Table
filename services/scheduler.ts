...
/**
 * PRIORITY-DRIVEN GRID SCHEDULING ALGORITHM v10.3
 * 
 * CORE LOGIC:
 * 1. Lunch is PRE-ALLOCATED based on Semester configuration.
 * 2. P1-P3 are MANDATORY. They must never be empty (unless lunch).
 * 3. No 3-Consecutive lectures for any teacher.
 * 4. Respects SEMESTER (Class-wise) lunch configurations.
 */
...
export const generateTimetable = (state: AppState): TimetableEntry[] => {
  ...
  const findBestCandidate = (sectionId: string, day: Day, slotIdx: number, forceFill: boolean) => {
  ...
  for (let slotIdx = 0; slotIdx < 3; slotIdx++) {
    DAYS.forEach(day => {
      sections.forEach(sec => {
        ...
        let match = findBestCandidate(sec.id, day, slotIdx, false);
        if (!match) match = findBestCandidate(sec.id, day, slotIdx, true); 
  ...
  for (let slotIdx = 0; slotIdx < totalSlots; slotIdx++) {
    DAYS.forEach(day => {
      sections.forEach(sec => {
        ...
        const match = findBestCandidate(sec.id, day, slotIdx, false);
        if (match) {
          const entry: TimetableEntry = {
